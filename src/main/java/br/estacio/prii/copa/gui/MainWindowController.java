...
//        AlertHelper.showWarning(null, "mnuUsersAction Performed");
        try {
            Pane newLoadedPane = FXMLLoader.load(getClass().getResource(Navigator.getUsers()));
            mainPane.getChildren().removeAll();
            mainPane.getChildren().add(newLoadedPane);
        }catch (IOException ex) {
            Logger.getLogger(MainWindowController.class.getName()).log(Level.INFO, ex.getMessage());
